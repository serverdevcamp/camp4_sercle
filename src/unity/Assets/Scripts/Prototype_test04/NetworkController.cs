/*
 * 1. 큐는 2개가 있다.
 * 2. 각 클라이언트에서 키 입력 정보를 매 프레임마다 서버에게 전송한다. 또, 큐1에 자기 정보를 넣는다.
 * 3. 전송받은 데이터를 서버는 저장했다가, 각 클라이언트에게 들어온 값을 전송한다.
 * -- 여기까지는 이전 채팅서버의 broadcasting과 같다 --
 * 4. 그러면 각 클라이언트는 큐1, 큐2에 각각 자신의 정보, 상대의 정보를 가지고 있게 된다.
 * 5. 큐 2개에 데이터가 차있다면 동기화 되었다는 뜻 이므로 게임 처리하면 된다.
 * 
 * -- 정정
 * 
 * 1. 서버단에서 큐 2개 할 필요 없음
 * 2. 각 클라이언트마다 본인은 0, 상대는 1의 인덱스로 Enqueue시 버퍼에 저장
 * 3. 
 */
...
public class NetworkController
{
	...
	public List<MouseData>[] mouseInputBuffer = new List<MouseData>[playerNum];
	...
	void SendInputData()
	{
		...
		int playerId = 0;
		int count = mouseInputBuffer[playerId].Count;
		...
		{
			inputData.datum[i] = mouseInputBuffer[playerId][i];

			//string str1 = "Send mouse data => id:" + i + " - " + inputBuffer[playerId][i].frame + " playerID: " + playerId.ToString();
			//Debug.Log(str1);
		}
	...
	public void DequeueMouseData()
	{
		...
		for (int i = 0; i < playerNum; ++i)
		{
			mouseData[i] = mouseInputBuffer[i][0];
			mouseInputBuffer[i].RemoveAt(0);

			//입력 관리자에게 동기된 데이터로서 전달합니다.
			m_inputManager.SetMouseInputData(i, mouseData[i]);
	...
	void EmurateInput()
	{
		...
		int opponent = (playerId == 0) ? 1 : 0;
		m_inputManager.SetMouseInputData(playerId, inputData);
		m_inputManager.SetMouseInputData(opponent, inputData);
