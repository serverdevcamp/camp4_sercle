...
namespace MagicalFX
{
	public class _Wizard : MonoBehaviour
	{
		...
		void Deploy ()
		{
			...
			_FX_Position fx = Skills [Index].GetComponent<_FX_Position> ();
			if (fx) {
				if (fx.Mode == _SpawnMode.Static) {
					Place (Skills [Index]);	
				}
				if (fx.Mode == _SpawnMode.OnDirection) {
		...
		void PlaceDirection (GameObject skill)
		{
			...
			_FX_Position fx = sk.GetComponent<_FX_Position> ();
			if (fx.Mode == _SpawnMode.OnDirection)
