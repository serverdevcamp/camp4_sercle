...
namespace MagicalFX
{
	public class _FX_LifeTime : MonoBehaviour
	{
		...
		public _FX_FadeToGround[] Faders;
		...
		void Awake ()
		{
			ten = this.GetComponent<_FX_Tentacle> ();
			if (Particles.Length <= 0) {
				Particles = (ParticleSystem[])this.transform.GetComponentsInChildren <ParticleSystem> ();
			}

			if (Faders.Length <= 0) {
				Faders = (_FX_FadeToGround[])this.transform.GetComponentsInChildren <_FX_FadeToGround> ();
			}
